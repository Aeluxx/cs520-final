...
export default function Edit() {
  ...
  const fetchNote = async () => {
    ...
    await axios.get<NoteType>('http://localhost:3000/api/note', { params: { id: query.id } }).then(res => {
  ...
  const updateNote = async (id: string, data: Partial<NoteType>) => {
    ...
    await axios.patch<NoteType>('http://localhost:3000/api/note', { id: query.id, ...data }).then(res => {
      setNote(res.data)
    })
  }
  ...
  useEffect(() => {
    if (!isReady || debouncedContent === null || debouncedContent === note?.content) return
  ...
  // Update note when content is changed
  useEffect(() => {
    if (!isReady || debouncedTitle === null || debouncedTitle === note?.title) return
  ...
  return (
    <Box
      ...
      <TextField onChange={e => setTitle(e.target.value)} value={title} size='small' sx={{ my: 1, display: 'block' }} />
      <Typography variant='caption'>Last updated {moment(note?.updatedAt).fromNow()}</Typography>
      <Editor
